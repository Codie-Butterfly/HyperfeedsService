...
public class BranchService {
    ...
    public List<BranchView> activeBranches() {
        return jdbc.sql("select id,code,name,address,phone_number,whatsapp_number,opening_hours,collection_enabled,active from branches where active order by name")
                .query(BranchView.class).list();
    }

    public BranchView branch(UUID id) {
        return jdbc.sql("select id,code,name,address,phone_number,whatsapp_number,opening_hours,collection_enabled,active from branches where id=:id and active")
    ...
    public BranchView create(BranchInput input) {
        ...
        jdbc.sql("insert into branches(id,code,name,address,phone_number,whatsapp_number,opening_hours,collection_enabled,active) values(:id,:code,:name,:address,:phone,:whatsapp,:hours,:collection,:active)")
                ...
                .param("whatsapp", input.whatsappNumber()).param("hours", input.openingHours())
    ...
    public BranchView update(UUID id, BranchInput input) {
        int changed = jdbc.sql("update branches set code=:code,name=:name,address=:address,phone_number=:phone,whatsapp_number=:whatsapp,opening_hours=:hours,collection_enabled=:collection,active=:active,updated_at=now() where id=:id")
                .param("id", id).param("code", input.code().trim().toUpperCase()).param("name", input.name().trim())
                .param("address", input.address().trim()).param("phone", input.phoneNumber())
                .param("whatsapp", input.whatsappNumber()).param("hours", input.openingHours())
                .param("collection", input.collectionEnabled()).param("active", input.active()).update();
    ...
    private BranchView branchForManagement(UUID id) {
        return jdbc.sql("select id,code,name,address,phone_number,whatsapp_number,opening_hours,collection_enabled,active from branches where id=:id")
    ...
    public record BranchView(UUID id, String code, String name, String address, String phoneNumber,
            String whatsappNumber, String openingHours, boolean collectionEnabled, boolean active) {}
